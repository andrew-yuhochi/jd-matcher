...
> **Last Updated**: 2026-05-01 (§5 M2 + §6 Scope OUT split: manual Expire button + Expired tab + `expired_canonicals` table promoted to PoC M3 per ALIGNMENT-LOG 2026-05-01 Override BA; auto-Inactivate + auto-Expire-via-HTTP-404 stay MVP-M1)
...
- Cross-state dedup generalised to canonical-id (a new posting matching an applied or dismissed canonical is suppressed from Main). **Exception**: a new posting matching a canonical that is currently in `Inactive` OR `Expired` state is NOT suppressed — both states are treated as non-existent for dedup purposes (URL-based and LLM content-based dedup paths). The new posting surfaces on Main with a fresh posting_id; the old Inactive/Expired entry remains visible (Applied tab Inactive sub-section for Inactive; Expired tab for Expired) as forensic history. **At M2** the dedup-bypass predicate is wired into the M2 dedup engine (TDD §C21 / §C22) as a no-op via the `WHERE status NOT IN ('Inactive', 'Expired')` guard — the `applied.status` enum in M1 has only `'Applied'`, so the guard matches everything. **Updated 2026-05-01 (Override BA per ALIGNMENT-LOG)**: PoC M3 introduces an `expired_canonicals` table (TDD §C2) plus a `NOT IN expired_canonicals` filter on the C21 BLOCK + FUSE candidate query (and C30 repost detector). This makes the Expired-side of the bypass **load-bearing in M3** for the manual Expire path (Expired tab + "Mark expired" button — TASK-M3-013); MVP-M1 reuses the same table for auto-Expire on hydrator HTTP 404 and additionally activates the Inactive-side of the status-based predicate.
...
- **Inactive state lifecycle (MVP-M1)**: auto-Inactivate `Applied`/`Screen`/`Interview` after ~90 days of silence on `status_updated_at`; dedup bypass for Inactive state; Applied tab Inactive sub-section; manual Inactive→any-status transition by user. Supersedes the original auto-remove design.
- **Auto-Expired state via hydrator HTTP 404 (MVP-M1)**: hydrator-detected dead-link postings auto-transition to `status='Expired'` on HTTP 404 from LinkedIn/Indeed; reuses the `expired_canonicals` table introduced in PoC M3 (see footnote). Auto-detection only — manual user-marked Expired is in PoC M3 scope.
  - **Footnote (2026-05-01)**: The **manual Expire button + Expired tab + `expired_canonicals` table + C21/C30 expired-filter activation** were promoted to **PoC M3 (TASK-M3-013)** per ALIGNMENT-LOG 2026-05-01 (Override BA). Rationale: dead-link postings poison the M3 evaluation testing loop (cannot distinguish ranker bugs from freshness issues during daily triage). MVP-M1 auto-detection (URL re-probe / HTTP 404) remains MVP-M1 scope and reuses the PoC-M3 table — purely additive.
