...
## 2026-05-01 — Add Expire tab + "Mark expired" button for closed/dead postings (proposed as TASK-M3-013)

**Verdict**: DRIFTING
**Mode**: B
**Anchors**:
- PRD §6 Scope OUT (Deferred to MVP): "Inactive AND Expired state lifecycle (auto-Inactivate `Applied`/`Screen`/`Interview` after ~90 days of silence on `status_updated_at`; auto-Expired on hydrator HTTP 404; dedup bypass for both states; Applied tab Inactive sub-section; Dismissed tab Expired sub-section; manual Inactive→any-status transition by user) — supersedes the original auto-remove design — MVP-M1." The Expire tab and manual-button are verbatim instances of "Dismissed tab Expired sub-section" and the manual-trigger variant of the Expired lifecycle, both of which are named in this clause.
- PRD §5 Scope IN M2 (dedup bypass wiring): "The dedup-bypass predicate is wired into the M2 dedup engine (TDD §C21/§C22) as a no-op at M2 … MVP-M1's status-enum extension flips the predicate to load-bearing without any C21/C22 code change." The PRD explicitly designates the C21/C30 Expired filter activation as MVP-M1 work, not PoC M3.
- ALIGNMENT-LOG 2026-04-26 ("Expired status for dead-link postings"): "manual 'Job link is dead' button deferred to MVP-M2 per BA recommendation" — user decision accepted and logged. The current proposal is the manual-button variant (user notices dead link on click-through), which is precisely the sub-case that prior verdict parked.
- PRD §4 Phase Objectives: "PoC validates the four capabilities that determine whether the rest of the system can be trusted." The four objectives are multi-source ingestion, content-aware dedup, LLM extraction/classification, and CV recommender. None depend on expired-state management being present in PoC.
- PRD §3 Commercial Thesis (Agreed): "Personal-first throughout PoC and MVP." Expired state management serves personal usability — it does not advance any of the three Beta gates (time savings, coverage superiority, generalisable personas) during PoC.

**Analysis**: The proposal is operationally motivated (dead links accumulate during active daily M3 triage) and the implementation is low-cost (~3-4 hours cloning the Dismissed pattern). However, the structural read is DRIFTING for two compounding reasons. First, the 2026-04-26 ALIGNMENT-LOG entry already classified the manual Expired button as MVP-deferred and the user accepted that verdict — promoting it to TASK-M3-013 reverses a logged user decision without a new PRD edit to update §6 Scope OUT. Second, PRD §6 Scope OUT explicitly names "Inactive AND Expired state lifecycle … Dismissed tab Expired sub-section … MVP-M1" as a compound lifecycle item; adding the Expired tab and button in PoC creates a PRD-vs-code inconsistency where half the lifecycle compound (auto-detect via hydrator HTTP 404 → MVP-M1) is still parked while the other half (manual tab + button) ships ahead of schedule. The workaround named in the 2026-04-26 entry — "users use Dismiss for dead links until MVP-M1 lands proper Expired handling" — still applies through M3. The DRIFTING (not VIOLATES) verdict reflects that the feature does not contradict the commercial thesis and does not touch any permanently-out clause; it is a time-placement issue, not a category issue. Pattern note: this is the first proposal in M3 to revisit a lifecycle item already logged as MVP-deferred. No escalating drift pattern observed across M3 yet, but this proposal establishes the first M3 boundary test.

**Recommendation**: DRIFTING — present the drift, then ask: "Override and add to TASKS as TASK-M3-013, or log to BACKLOG.md for MVP-M1 (bundle with existing Inactive AND Expired state lifecycle entry)?"

Preferred landing zone if user parks: bundle into the existing BACKLOG.md "Inactive AND Expired state lifecycle — MVP-M1" entry. The manual-button variant and the Expire tab are already partially described in that entry; this proposal adds the `expired_canonicals` table design (mirroring `dismissed_canonicals`), the C21/C30 filter-clause activation, and the `select_expired` query mirror — all of which belong in the same MVP-M1 implementation block.

Preferred framing if user overrides: architect must update PRD §6 Scope OUT (remove the Expired tab + manual-button from the MVP-M1 compound clause) and TDD (activate the C21/C30 no-op predicate as load-bearing in M3 rather than MVP-M1, and document the new `expired_canonicals` table alongside `dismissed_canonicals`) before TASK-M3-013 is drafted. The BACKLOG.md "Inactive AND Expired state lifecycle" entry must also be scoped down to remove the Expired sub-items now landing in M3.

**User decision**: **Override BA — promote to TASK-M3-013** (decided 2026-05-01).

**User's stated reasoning**: dead/expired postings poison the M3 evaluation testing loop. When a posting ranks highly but the apply link is dead, the user cannot tell whether the issue is a ranker quality bug or a freshness issue — the two confounds are indistinguishable without an explicit "this posting is dead" signal during daily triage. The Expire feature is therefore part of the testable PoC logic (it cleans the M3 evaluation surface), not UX polish. The 2026-04-26 deferral assumed Expire was a comfort feature; M3 daily-triage usage has reframed it as a quality-evaluation prerequisite. Implementation cost (~3-4h cloning the Dismiss pattern) is acceptable for the M3 evaluation gain.

**Doc restructuring committed in this transaction (architect, 2026-05-01)**:
1. **PRD §6 Scope OUT** — the "Inactive AND Expired state lifecycle" clause split: Inactive lifecycle (auto-Inactivate after 90 days silence, Applied tab Inactive sub-section) stays in MVP-M1 deferral; auto-Expired via hydrator HTTP 404 stays in MVP-M1 deferral; **manual Expire button + Expired tab moved out of the deferral and into PoC M3** with cross-reference to this entry.
2. **TDD §C21 (Stage-1 BLOCK + dispatch) + §C30 (repost detector)** — the previously documented `WHERE NOT EXISTS … applied.status IN ('Inactive', 'Expired')` no-op predicate is **superseded in M3 by an `expired_canonicals` table check**. C21 BLOCK + FUSE candidate query and C30 repost-detector both add `AND canonical_id NOT IN (SELECT canonical_id FROM expired_canonicals WHERE user_id = ?)`. The MVP-M1 status-based `applied.status IN ('Inactive', 'Expired')` predicate stays documented as the additional filter MVP-M1 will layer on for the auto-detection path; it is not replaced, it is complemented.
3. **TDD §C2 (schema)** — `expired_canonicals (canonical_id INTEGER NOT NULL, user_id TEXT NOT NULL DEFAULT 'default', marked_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, marked_by_user TEXT NOT NULL DEFAULT 'default', PRIMARY KEY (user_id, canonical_id), FOREIGN KEY (canonical_id) REFERENCES canonical_postings(canonical_id) ON DELETE CASCADE)` table added with index `idx_expired_canonicals_user (user_id, marked_at DESC)`. **Architectural surprise**: the user's context message referenced `dismissed_canonicals` as the schema mirror, but the current architecture has NO such table — Dismiss state lives on the per-posting `dismissed` table joined through `posting_canonical_links` for canonical-level inheritance on read. The new `expired_canonicals` is therefore a NEW design pattern (canonical-keyed direct, NOT per-posting like `dismissed`). The asymmetry is documented in §C2 as deliberate. Migration: new table added to `src/jd_matcher/db/schema.sql` (the existing startup `executescript(schema_sql)` makes `CREATE TABLE IF NOT EXISTS` idempotent); `_apply_pending_migrations` is NOT used — that helper handles only ALTER TABLE column adds.
4. **TDD §C7 + §C22** — `expire_canonical(canonical_id)` and `unexpire_canonical(canonical_id)` write API on C7 (mirroring `dismiss_canonical`); `select_main` predicate extended with the expired check (mirrors the existing dismissed predicate); new `select_expired` read on C22 (mirrors `select_dismissed`).
5. **TDD §C8 + §C9** — `POST /postings/{id}/expire` + `POST /postings/{id}/unexpire` endpoints; "Expired" tab nav slot with count badge (mirrors Dismissed tab plumbing from M2-013); red "Mark expired" button on each Main card (mirrors the Dismiss button placement).
6. **TDD §C30 — no-linkage clause added**: when a fresh posting arrives matching the role of an expired canonical, the dedup engine (C21) treats the expired canonical as if it does not exist (per the table-based filter). The fresh posting becomes a NEW canonical; C30 does NOT tag it as a "Reposted" variant of the expired canonical. Clean separation — no "Reopened" chip, no metadata linkage between expired and fresh canonicals. The user's reasoning: an expired canonical is forensic history, the fresh canonical is a new candidate; conflating them re-introduces the ranker-vs-freshness confound the Expire feature exists to eliminate.
7. **BACKLOG.md** — the "MVP-M1 — Inactive AND Expired state lifecycle" entry trimmed: manual Expire button + Expired tab + `expired_canonicals` table removed (now PoC M3); auto-Inactivate (90-day silence) + auto-Expire via hydrator HTTP 404 + Inactive tab/section + dedup-bypass-on-status retained as MVP-M1 scope. The existing manual "Job link is dead" item (deferred to MVP-M2 in 2026-04-26) is also removed and superseded by this M3 promotion. Cross-reference updated to point here.
8. **TASKS.md** — TASK-M3-013 inserted with Implementation Checklist + 10 ACs covering schema migration, C21/C30 filter, C7 write API, C22 read API, C8 endpoints, C9 nav slot + button, integration test of the reactivation cycle.

**MVP-M1 contract preserved**: the auto-Inactivate (90-day silence) + auto-Expire-on-HTTP-404 work stays in MVP-M1. PoC M3 ships the user-driven manual Expire flow only; the auto-detection layer arrives at MVP-M1 and reuses the same `expired_canonicals` table that PoC M3 introduces (so MVP-M1's auto-Expire is purely additive — it inserts into the same table from a different trigger).

**Pattern note for future BA invocations**: this is the first M3 promotion that reverses a previously logged MVP deferral. The override is justified by a re-frame (UX polish → quality evaluation prerequisite), not by lowered scope discipline. If a second M3 proposal cites the same "needed for evaluation" framing, BA should probe whether the M3 evaluation surface itself is under-specified rather than auto-promote.

**Outcome**: TASK-M3-013 promoted to PoC M3. Doc restructuring (1)–(8) committed in the same transaction as this log entry. TDD updated to make the M3 expired filter load-bearing. PRD §6 Scope OUT split between PoC M3 (manual Expire) and MVP-M1 (auto-detection lifecycle). BACKLOG entry trimmed. Implementation begins via `/implement jd-matcher TASK-M3-013` after the user finishes the earlier M3 tasks (current next task: TASK-M3-001).
