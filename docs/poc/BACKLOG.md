...
- **`extraction_cache` → `canonical_postings` propagation gap (audit needed)** — surfaced 2026-04-29 during TASK-M2-012 final calibration. Jobright canonicals 316/395/396/458 had successful LLM extraction in `extraction_cache` but `top_skills`/`canonical_seniority`/`role_summary` were never propagated to `canonical_postings` (forced manual UPDATE during M2-012). Suggests the canonicalization step has a gap where cache hits don't always propagate. Audit: scan all canonicals for `top_skills='[]'` AND non-empty cache rows; quantify scope. May affect future ingests silently.
