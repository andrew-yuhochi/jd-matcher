...
| Field | Value |
|-------|-------|
| **Input** | An email-extracted `title` string (from C4's `ParsedPosting.title`) for each new posting, plus the candidate's `(source, source_job_id, url)` so the orchestrator can short-circuit cleanly. Configuration: deny-list patterns (regex or substring) and allow-list overrides loaded from `config/title_filters.yaml`. |
