...
| **Output** | A `GatekeeperVerdict` Pydantic model `{is_same_role: bool, reasoning: str, model_name: str, latency_ms: int, cost_usd: float}` returned synchronously to C21. C21 then uses the verdict to set the final `DedupDecision.action`: **LLM `is_same_role=True` AND FUSE ≥0.75 → `action='merge'`**; **LLM `is_same_role=False` → `action='new'`** (verdict OVERRIDES the FUSE proposal); **FUSE <0.75 → gatekeeper not called, default no-merge per C21**. Side-effect: writes one `llm_call_ledger` row with `call_kind='dedup_gatekeeper'`, `notes` containing both posting_ids and the FUSE score that triggered the call. Per-pair verdict + reasoning logged at DEBUG level for user audit. |
| **Responsibility** | (1) Public API: `LLMDedupClassifier.classify(posting_a: Posting, posting_b: Posting) -> GatekeeperVerdict` in `src/jd_matcher/dedup/classifier.py`. (2) **Provider abstraction:** uses C28's `LLMExtractor` interface — NO direct `openai`/`anthropic` SDK imports in this module. The same `get_extractor(config)` factory that powers C18 powers C32; the swap to Ollama/local is config-only. (3) **Prompt:** new file `prompts/dedup_classifier_v1.txt`. The prompt asks: *"Are these the same role at the same employer?"* Inputs interpolated into the prompt: each posting's full `full_jd` text + `canonical_title` + `canonical_company`. **Why FULL JDs and not `role_summary`:** for hiring-agent and multi-role postings, `role_summary` strips the team/responsibility detail that distinguishes "Senior DS — Risk Modelling" from "Senior DS — Marketing Analytics" at the same company. The gatekeeper's job is over-merge protection on the borderline 0.75–0.95 FUSE band; that's exactly the band where role_summary collapses too much signal. The token cost per pair is higher (full-JD pair averages ~3–6k input tokens vs ~600 tokens for summaries) but it matches the actual user value being protected (per ALIGNMENT-LOG 2026-04-29). (4) **Strict JSON output contract:** the prompt requests a JSON object with fields `is_same_role: bool` and `reasoning: str` (1–2 sentences). The raw response string is parsed and validated against the `GatekeeperVerdict` Pydantic model. JSON parse / schema validation failure → raise `GatekeeperParseError` (Minor tier — caught by C21, treated as "no verdict, default to FUSE proposal at 0.90 auto-merge threshold" so the gatekeeper is never a hard dependency that breaks dedup; logged as a parse-rate metric). (5) **Cost & telemetry:** every call writes `llm_call_ledger(posting_id=NULL, call_kind='dedup_gatekeeper', model=…, prompt_tokens, completion_tokens, cost_usd, latency_ms, status='success'|'parse_error'|'api_error', notes=JSON({"posting_a": …, "posting_b": …, "fuse_score": …, "verdict": …}))`. (6) **No write side-effects on the dedup tables** — C32 is a pure decision function; the merge action remains C29's responsibility. C21 receives the verdict and forwards an updated `DedupDecision` to C29 as before. |
