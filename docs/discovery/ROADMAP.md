...
1. Multi-source ingestion (Gmail-driven LinkedIn/Indeed/Job Bank email parsing + JD hydration; direct API polling for Himalayas/Remotive/Jobicy/HN)
...
|---|-----------|---------------------|------------------|--------------|
| 1 | Ingestion + JD hydration | URL extraction from LinkedIn/Indeed alert emails; full-JD hydration via guest endpoints; structured API calls to Himalayas/Remotive/Jobicy/HN succeed | LinkedIn/Indeed: ≥95% URL extraction on ≥50 sample emails. JD hydration: ≥95% successful full-JD fetch (≥30 samples). API sources: ≥95% successful structured fetch (deterministic). | M1, M4 |
...
| **User-observable deliverable** | Same web app as M1, but now: (a) duplicate postings across LinkedIn + Indeed + Himalayas are visibly merged into a single card with multiple `Sources:` listed; (b) when one variant is dismissed or applied, the canonical record's state suppresses all matched variants from Main on subsequent runs (state inherited from M1, now keyed off canonical-id rather than just URL); (c) reposted JDs surface as a single card with original `first_seen` preserved. Also: Himalayas API source added (≥90 Canadian DS results — see RESEARCH-REPORT.md §5). |
...
| **Acceptance criteria** | (1) ≥90% accuracy on 30 hand-labeled posting pairs (10 dup / 10 non-dup / 10 ambiguous). (2) **Zero false-merges** on the 10 different-team cases (regression-blocking). (3) Cross-source merge: a single JD posted on LinkedIn AND Indeed appears as one card with both URLs in `sources[]` (verified on ≥3 real cross-source pairs). (4) State inheritance generalizes from M1: dismissing one source's variant suppresses the canonical record across all sources on the next run. (5) Repost detection: same JD with new `jobId` after 30+ days → merged with preserved `first_seen` (verified on ≥3 real cases or synthetic). (6) Auto-merge threshold 0.90 calibrated against the hand-labeled set, recorded in config. |
...
| Field | Value |
|-------|-------|
| **Goal** | User sees a CV recommendation per posting and can override. Coverage expands from LinkedIn + Indeed + Himalayas to also include Job Bank Canada, Remotive, Jobicy, HN. Settings page exists. Time-savings analytics view exists. |
