...
| 3 | LLM extraction + classify + fit | One prompt extracts canonical fields, salary, fit score, role orientation, industry, citizenship, Canadian-hire | `role_orientation` set-equality agreement ≥80%; `fit_score` accept/reject agreement ≥90% at user-set N (default N=3 on 1-5); salary ≥90% within ±10%; `industry` ≥75%; `citizenship_requirement` ≥90% (3-state); `can_hire_in_canada` ≥85% (4-state); hard-filter 0 false-negatives on 10 eligibility-blocking cases (probabilistic per-field — user approval gate; hard-filter is deterministic regression-blocking) | M3 |
...
| Field | Value |
|-------|-------|
| **Goal** | Cards arrive pre-classified, pre-scored, pre-filtered. User opens app and immediately sees a sorted shortlist (DS-fit, orientation diversity, salary, recency); ineligible / low-fit roles are queryable but tucked behind a Filtered tab. |
| **User-observable deliverable** | Web app now shows on every Main card: industry chip on the location row; a fit-score chip (1-5) with `fit_reasoning` tooltip; salary chip; role_orientation chip (1-3 of Engineering/Problem-Solving/Communication); citizenship + Canadian-hire badges in the source row. Cards are deterministically tuple-sorted DESC by `(fit_score, orientation_diversity, salary_max_cad, post_date)`. New 4th tab **Filtered** with count badge — surfaces canonicals with `is_filtered=1`, each card showing a `Filtered: <reason>` badge and a "Show anyway" override that flips `is_filtered=0` and logs a `card_filter_overridden` event. After M3 first pass on 148 canonicals: industry-taxonomy revision pass (rename/merge/split with user approval, prompt updated, affected canonicals re-extracted). |
| **Critical components built** | Extend C18 (single LLM extraction prompt, cloud-default OpenAI `gpt-4o-mini`, ~$0.60/month at 20 postings/day; config-swappable to local Ollama `qwen2.5:7b`) with seven new fields: `fit_score` (1-5), `fit_reasoning`, `industry` (16-sector closed taxonomy: Financial Services / Asset Management; Insurance / Insurtech; Telecom / Digital Services; Gaming / Entertainment; Legal Tech / Compliance; Professional Services / Consulting; Construction / AEC; Energy / Oil & Gas / Cleantech; AI Training / Annotation Platforms; Staffing / Recruiting; AdTech / Marketing Tech; B2B SaaS; Healthcare / Healthtech; Retail / Ecommerce; Government / Public Sector / Crown Corp; Other), `role_orientation` (1-3 of Engineering / Problem-Solving / Communication — replaces the obsolete `primary_focus`), `salary_min_cad` + `salary_max_cad` (CAD-normalized), `citizenship_requirement` (required / preferred / not_mentioned, 3-state) + `citizenship_reason`, `can_hire_in_canada` (yes / likely / no / unclear, 4-state). New C33 — Hard Filter Engine (post-LLM, null-tolerant; reads `user_profile.yaml::hard_filters`; runs after C18/C20/C21/C29; writes `is_filtered` + `filter_reason` on `canonical_postings`). New C34 — Card Ranker (read-only deterministic 4-tuple sort key for Main view ordering). C8 endpoint extensions (`GET /filtered`, `POST /postings/{id}/filter_override`). C9 frontend extensions (Smart Layer chip row, Filtered tab + count badge, override button, fit_reasoning tooltip). C10 event extension (`card_filter_overridden`). C11 pipeline orchestrator refactor (Path A: decompose 1480-line `pipeline.py` into per-phase modules under `pipeline/phases/{fetch,parse,filter,hydrate,extract,embed,dedup,merge,hardfilter,rank}.py`) PLUS C18→postings propagation fix folded in (closes Jobright bug class). C22 Canonical View extension (`select_main` applies `is_filtered=0` filter + ranker; new `select_filtered`). Schema additions to `canonical_postings` (extends C2): the seven LLM fields + `is_filtered` + `filter_reason`; ~148 existing canonicals back-filled by re-extraction (~$0.15). Generic role-family-language prompt (hedge 4) preserved. Hedge-1 below-threshold visibility proven via the Filtered tab. Closing M3 task: industry-taxonomy revision pass + prompt update + selective re-extraction. **Optional**: local-vs-cloud benchmark sub-task (Ollama `qwen2.5:7b` vs `gpt-4o-mini`) — opt-in only; not a default M3 deliverable. |
| **Acceptance criteria** | (1) `role_orientation` ≥80% set-equality agreement with user labels on ≥30 hand-labeled postings using cloud `gpt-4o-mini`. (2) `industry` ≥75% exact-match agreement on the same set (lower bar than role_orientation since 16-class is harder than 3-class). (3) `fit_score` ≥90% accept/reject agreement at user-set threshold N (default N=3 on the 1-5 scale). (4) Salary extraction ≥90% within ±10% on the labeled subset where salary is stated. (5) `citizenship_requirement` ≥90% exact-match (3-state). (6) `can_hire_in_canada` ≥85% exact-match (4-state). (7) Hard-filter post-LLM rejects all 10 citizenship-required-AND-user-not-eligible test cases (zero false-negatives on the eligibility-blocking subset). (8) Filtered tab visible — count matches `WHERE is_filtered=1`; "Show anyway" override flips `is_filtered=0` and writes a `card_filter_overridden` event. (9) Cloud-LLM cost report: extraction + embedding + back-fill cost logged to `quality-logs/llm-cost.md`; expected ≤$1/month running + ~$0.15 one-shot back-fill of 148 canonicals. (10) Industry taxonomy revision pass: distribution tabulated, user-approved rename/merge/split applied to the prompt, affected canonicals re-extracted. (11) **User approves probabilistic outputs** before milestone closes. (12) **Optional, only if opted in**: local-vs-cloud benchmark report in `quality-logs/llm-benchmark.md`. |
| **Quality bar** | Probabilistic per-field — fit_score / role_orientation / industry / citizenship / Canadian-hire flagged for user approval per Gate 4. Hard-filter has a regression-blocking deterministic gate (zero false-negatives on eligibility-blocking JDs). Salary extraction has both a deterministic threshold (≥90% within ±10%) and user review of failure cases. Validation is against cloud `gpt-4o-mini` extraction quality on the hand-labeled sample. |
| **Dependencies** | M2 (postings must already be deduplicated; LLM call runs on canonical records). M3 also folds in TASK-M3-000 cleanup per ARCHITECTURE-REVIEW-2026-04-29 — C18→postings propagation fix, pipeline.py decomposition, TDD footnote consolidation, drop dead `dedup.auto_merge_threshold` field — bundled with the first user-visible M3 feature so the "no pure infra milestones" rule holds. |
...
PoC (active phase next)
  ...
  Critical components:
    ...
    - LLM extract+classify+fit — ≥80% role_orientation, ≥90% fit accept/reject (1-5/N=3),
                                  ≥75% industry, ≥90% citizenship 3-state, ≥85% Canadian-hire 4-state
  ...
  Milestones:
    ...
    M3 — Cards arrive pre-classified, pre-scored, pre-filtered (cloud LLM); Filtered tab
